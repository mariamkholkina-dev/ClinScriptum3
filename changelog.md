...
### Feat: перетаскиваемый разделитель панелей (ревью + внутридокументный аудит)

На экране ревью находок (rule-admin finding-review) и на экране внутридокументного аудита (web) ширина левой панели списка регулируется перетаскиванием разделителя между списком и детализацией. Курсор col-resize, ограничения по минимальной ширине обеих панелей; правая панель — `min-w-0`, контент занимает всю ширину окна. Только UI (web + rule-admin), backend не затронут.
