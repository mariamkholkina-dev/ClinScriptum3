...
### Feat: форматированные (HTML) разделы в детализации внутридокументного аудита (web)

`getDocumentSections` теперь возвращает `contentHtml` (из `rawHtml` блоков) — на экране внутридокументного аудита (web, writer) разделы в детализации находки показываются форматированными (таблицы/переносы), как на экране ревью (rule-admin). Раньше грузился только plain-text (`select: content`), HTML не было. Объём ≈ размер документа (как в `getReview`); тяжёлый `digitalTwin` не тянем. Требует деплой api + жёсткий рефреш web.

### Fix: декодирование HTML-сущностей в тексте разделов (ложные находки + переход)

`stripHtml` в doc-parser (parser.ts и table-parser.ts) убирал теги, но не декодировал HTML-сущности (`&gt;`, `&lt;`, `&amp;`, `&quot;`, числовые) — только `&nbsp;`. В итоге в `ContentBlock.content` (текст, который видит LLM и который используется как цитата) попадал, например, `&gt;` вместо `>`. Последствия: (1) ложные находки вида «использована HTML-сущность &gt; вместо '>'»; (2) цитата с `&gt;` не совпадала с текстом документа → не работал переход в Word. Добавлен общий `decodeHtmlEntities` (`&amp;` декодируется последним — без двойного декодирования), применён в обоих `stripHtml` + тест. Требует деплой workers + **переобработку** уже загруженных версий (новые парсы корректны сразу).

## 2026-05-31

### Fix: отчёт внутридокументного аудита выгружался пустым

`/api/audit-report/:versionId` фильтровал находки по `type="intra_audit"` ИЛИ `type="editorial" + issueFamily="EDITORIAL"`, но боевой аудит пишет находки с типами `editorial`/`semantic` → `semantic` и большинство `editorial` в отчёт не попадали (пустой отчёт при наличии находок на экране). Запрос приведён к тому же набору, что `getAuditFindings`: `type IN (intra_audit, editorial, semantic)`, с исключением `false_positive` и `hiddenByReviewer` (как видит писатель). Требует деплой api.
### Fix: Word add-in — «К списку» в фиксированной панели + различимые цитаты

- Кнопка **«К списку»** перенесена в фиксированную верхнюю панель навигации (раньше была в прокручиваемой области и обрезалась) — слева, рядом с «← Назад / N/M / Вперёд →».
- **Две цитаты** визуально разделены: разный цвет левой полосы (по индексу), отступ между ними и подпись «Цитата 1/2» — больше не выглядят как одна ссылка.

Требует rebuild word-addin.

### Feat: reviewer (rule-admin) — разделы документа в HTML (таблицы, переносы строк)

В детализации находки на экране ревью (rule-admin finding-review) разделы документа теперь рендерятся как **HTML** (из `ContentBlock.rawHtml`), а не плоский текст. Видны таблицы и переносы строк — устраняет ложные находки «нет пробела», когда на деле это новая ячейка/строка таблицы.

- Backend: `getReview` отдаёт `contentHtml` раздела (хелпер `buildSectionHtml` из `rawHtml` блоков; fallback — экранированный `<p>`).
- UI: общий `SectionPanel` при наличии `contentHtml` рендерит санитизированный HTML (вырезаны script/style/iframe/обработчики событий) со стилизацией таблиц/абзацев/списков; подсветка цитат остаётся в text-режиме (когда HTML нет).
- `getDocumentSections` (writer-аудит) НЕ трогали — он намеренно не грузит `rawHtml` (защита от heap-OOM на больших документах).

Требует деплой api + rule-admin.
### Fix: Word add-in — пустая область (Divider) и потеря второй цитаты в ревью

- **Пустая область между фильтрами и списком** вернулась после фикса прокрутки: Fluent `<Divider>` в колоночном флексе растягивался (`flexGrow`) и съедал свободную высоту. Divider убран, разделение — через `borderTop` у списка; список заполняет высоту и прокручивается.
- **Показывалась только одна цитата** у cross-check находок: вторая цитата лежит в `referenceQuote` (а не `targetQuote`), и сборщик `quoteList` брал её лишь как fallback. Теперь собираются оба места (referenceQuote/anchorQuote + textSnippet/targetQuote) + остальные непустые цитаты — обе цитаты видны и кликабельны.
- **Переход в документ привязан к разделу**: раньше `navigateToText` выделял ПЕРВОЕ вхождение текста по всему документу. Теперь находим заголовок раздела находки (по разделу, содержащему цитату, с предпочтением зоны находки — из `getReview.sections`) и выбираем первое вхождение цитаты ПОСЛЕ заголовка (`Range.compareLocationWith`).

Требует rebuild word-addin.

### Feat/Fix: Word add-in — доработки экрана «Ревью находок»

- **Навигация** по находкам в детализации (← Назад / Вперёд → + счётчик «N / M») по текущему отфильтрованному списку; кнопки блокируются на границах.
- **Бэйдж статуса «Ложное»** (`false_positive`) теперь красный (`danger`) вместо бледно-серого.
- **Фикс прокрутки** списка находок: корень `ReviewPanel` переведён на `height:100% + minHeight:0` (как в ParsingPanel) — `flex:1` внутри нефлексового `content` ломал скролл (новые находки не появлялись при прокрутке).
- **Бэйдж «Направление»** (self/cross/editorial) в карточке и детализации + соответствующий фильтр (как в web).
- **QA-верификация**: в детализации показываются вердикт и причина QA (`qaVerdict`/`qaReason`) — важно для ложноположительных (видно, почему QA отклонил).
- **Массовые операции**: в bulk-баре добавлены «Выбрать все (N)» и «Снять все».
- **Счётчик по фильтру** в шапке: «По фильтру: K из N».
- **Статус-фильтр** в ревью больше не дублирует «Ложное срабатывание» (его покрывает фильтр видимости).
- **Intra-audit (FindingsFilter)**: убрана опция статуса «Ложное» — писатель ложноположительные не видит (паритет с web).
- **Подписи разделов** детализации: «Рекомендация», «Цитаты из документа», «QA-верификация».
- **QA-бэйдж** цветной по вердикту: подтверждено — зелёный, отклонено — красный, скорректировано — жёлтый.
- **Зона/зоны** находки в детализации (Зона: X / Зоны: anchor → target).
- **Фикс «Перейти в документе»**: `navigateToText` ронял поиск на длинной цитате (>255 символов — лимит Word.search) без try/catch → клик молча не срабатывал. Теперь term режется до 255 и каждый кандидат в try/catch. NB: переход ищет по открытому в Word документу — ревью открывать через «Открыть в Word».
- **Навигация к месту в документе**: при открытии карточки — автопереход к первому месту находки; цитаты кликабельны (переход к конкретному месту, текущее подсвечено). Отдельная кнопка «Перейти в документе» убрана — переход только по клику на цитату.

Требует rebuild word-addin.

### Fix: Word add-in — пустая область между фильтрами и списком в ревью находок

У `ReviewPanel` корневой контейнер был `height: 100%`, из-за чего внутри нефлексового `styles.content` высота не схлопывалась и между фильтрами и списком находок зияла большая пустая область. Заменено на `flex: 1 + minHeight: 0` (как в `FindingsPanel` внутридокументного аудита, где этот баг уже был исправлен). Требует rebuild word-addin.

### Feat: «Открыть в Word» для ревью находок (rule-admin → Word add-in)

На экране ревью находок rule-admin добавлена кнопка **«Открыть в Word»** — открывает текущее ревью в Word-аддоне (режим «Ревью находок»).

- Word-сессия теперь несёт `reviewId` и режим `finding_review` (`WordSessionContext` + REST `/api/word-sessions` + exchange). Аддон по auto-auth получает context и сразу открывает `ReviewPanel` нужного ревью (без ручного выбора).
- rule-admin `openInWord` расширен режимом `finding_review` + `reviewId`; кнопка использует `review.docVersionId` (в этот документ впрыскивается session-tag для открытия .docx).

Требует деплой api + rule-admin + rebuild word-addin.

### Fix: детализация находки показывает только разделы её зон(ы)

`selectTestedSections` искал разделы по совпадению цитаты по ВСЕМ разделам документа — один и тот же фрагмент встречается в нескольких разделах, поэтому в детализацию попадали разделы из чужих зон. Теперь кандидаты сначала ограничиваются зонами находки (`anchorZone`/`zone` = rootZone = первый компонент `standardSection`), и уже внутри них сужаются по цитате/названию; если узкого совпадения нет — показываются разделы этих зон. Для находок без зоны поведение прежнее. Поправлено в общем `finding-display` (web-аудит и rule-admin finding-review).

### Fix: фильтр «Ложное срабатывание» в finding-review ловит и status=false_positive

На экране ревью замечаний (rule-admin и Word-аддон) фильтр «Ложное срабатывание» проверял только `hiddenByReviewer` и не показывал находки, помеченные конвейером/LLM (`status=false_positive`), хотя на карточках они тоже подписаны «Ложное срабатывание». Теперь фильтр считает находку ложноположительной, если `hiddenByReviewer === true` ИЛИ `status === "false_positive"` (а «Не ложные» — отрицание обоих).

### Feat: Word add-in — возврат ложноположительной находки на валидацию

В режиме «Ревью находок» Word-аддона добавлено восстановление ошибочно помеченных `false_positive` находок (паритет с rule-admin): кнопка **«Вернуть на валидацию»** в детализации (для FP-находок) и **«На валидацию»** в bulk-баре. Использует `findingReview.restoreFromFalsePositive`. Требует rebuild word-addin.
...
- **Promote-to-golden («В эталон»):** в детализации находки и в bulk-баре — кнопка переноса находки(ок) в эталонный набор. Новый reviewer-доступный эндпойнт `findingReview.listGoldenSamples` (минимальный список наборов; не открывает ревьюеру quality-only `goldenDataset.listSamples`); перенос — через существующий `promoteFindingToGolden`. Модалка `PromoteToGoldenModal` (web) поддерживает одну находку и массовый перенос.
- **Массовые операции:** чекбоксы у находок + «Выбрать все (по фильтру)»; bulk-бар: скрыть (FP) / показать / сменить серьёзность / в эталон. Новые эндпойнты `findingReview.bulkSetHidden` и `bulkChangeSeverity` (пишут audit-лог по каждой изменённой находке; severity — и в колонку, и в `extraAttributes`, + `originalSeverity`).
- **Завершение ревью:** кнопка переименована «Опубликовать» → **«Завершить ревью»**, текст подтверждения — «Завершить ревью и опубликовать находки писателю» (механизм тот же `publish`; отдельная кнопка не требовалась — публикация и есть подтверждение завершения).
- Заметка ревьюера — уже была, паритет с rule-admin сохранён.

Требует деплой api + web. Тесты сервиса finding-review дополнены (listGoldenSamples, bulkSetHidden, bulkChangeSeverity).

### Feat: унификация экранов находок (внутридокументный аудит ↔ finding-review) + скрытие ложноположительных от писателя

Привели экран ревью находок (`finding-review`, web) к виду экрана внутридокументного аудита и устранили рассинхрон серьёзностей.

- **Общий модуль `apps/web/src/components/finding-display.tsx`** — вынесены все константы (severity/status/type/zone/issue-family labels и стили), `extractFindingMeta`, `effectiveSeverity`, бэйджи (`FindingBadges`), тело карточки (`FindingCardBody`), богатая детализация (`FindingDetailBody`) и `SectionPanel`. Оба экрана (audit, finding-review) рендерят находки одинаково из одного источника.
- **finding-review (web):** в списке — полные карточки как на аудите (бэйджи severity/метод/направление/семейство/QA + рекомендация + зоны + превью цитаты) + признаки ревью (ложное срабатывание, «Алгоритм: X», заметка). Детализация через общий `FindingDetailBody` + панель действий ревьюера. Добавлен фильтр «ложное срабатывание» (видимые/скрытые); severity- и статус-фильтры показывают **все** возможные значения, даже отсутствующие в наборе.
- **Соответствие серьёзностей:** у intra-audit находок колонка `Finding.severity` не заполняется (реальная severity в `extraAttributes.severity`), из-за чего на finding-review всё показывалось как INFO. Теперь обе поверхности читают «эффективную» severity одинаково, а `changeSeverity` пишет правку И в колонку, И в `extraAttributes.severity` (+ сохраняет `originalSeverity` при первой правке) — правка ревьюера отражается на экране аудита и в отчёте.
- **Ревьюер видит все находки**, включая помеченные `false_positive` (insufficient_context / QA-dismissed / дедуплицированные) — `getReview` больше не отфильтровывает их (фильтр на клиенте).
- **Писатель НЕ видит ложноположительные** — `getAuditFindings` для не-ревьюера исключает `status=false_positive` (как и `hiddenByReviewer`); на экране писателя убрана опция «Ложное срабатывание» из статус-фильтра.
- **Детализация — только тестируемые разделы:** вместо всех разделов зоны показываются только те, чей контент реально содержит цитату находки (`selectTestedSections`); зона — последний fallback.
- **Выделение цитаты — полная цитата:** подсветка больше не режется до 80 символов; цитата бьётся по «…», каждый фрагмент подсвечивается целиком, пробелы/переносы в regex гибкие (`\s+`).
- **getReview** теперь возвращает `sections` документа (для детализации на finding-review).

Требует деплой api + web. Тесты сервисов обновлены (finding-review, audit).
...
- **Экран внутридокументного аудита:** три фильтра (серьёзность/тип/статус) не помещались в ширину панели списка — последний обрезался. Контейнер переведён на `grid grid-cols-2` (перенос 2+1), селекты `min-w-0`.
- **Экран ревью находок (`finding-review/[reviewId]`):** добавлены фильтры по типу и статусу (раньше был только по серьёзности) — аналогично экрану аудита, в `grid grid-cols-2`.
- **Все находки на ревью показывались как INFO** + неверные счётчики: страница читала `severity` из колонки, а у intra-audit находок реальная серьёзность в `extraAttributes.severity`. Введён `effSeverity()` (override ревьюера → extraAttributes → info), применён в фильтре, счётчиках и карточках/детали.

Только `apps/web/.../audit/[docVersionId]/page.tsx` и `apps/web/.../finding-review/[reviewId]/page.tsx`. Требует деплой web.

### Fix: подписи бейджей на экране аудита (severity/тип/направление путались)

На карточке и в детали находки бейджи «направление» (taskKind, напр. «Перекрёстная проверка») и «тип» (Семантическая/Редакторская) выглядели как одинаковые серые пилюли и читались как «типы находки». Добавлены подписи: бейдж типа → «Тип: …», бейдж направления → «Направление: …» (на карточке и в детали). Только `apps/web/src/app/(app)/audit/[docVersionId]/page.tsx`. Требует деплой web.

### Fix: UX панели находок в Word add-in (5 пунктов)

- **«Подсветить все» не работала** для части находок — собирала цитаты только из `textSnippet`, тогда как у intra-audit находок текст лежит в `anchorQuote`/`targetQuote`/`checkedDocQuote`. Введён общий `bestSnippet()` — перебирает все поля.
- **Счётчик находок показывал «99+»** (cap Fluent `CounterBadge`) → `overflowCount={99999}`, теперь точное число.
- **Навигация Вперёд/Назад** на экране находки: кнопки + индикатор «N / M», переключение по отфильтрованному списку.
- **Список не скроллился мышью** — flex-ребёнку списка/детали не хватало `minHeight: 0` (overflow не срабатывал) → добавлено.
- **Переход к месту в документе** улучшен: нормализация пробелов/переводов строк под `Word.search` (фрагменты через границы абзацев теперь находятся), длина ≤200, fallback на более длинные префиксы (120/60 вместо 40 — меньше ложных совпадений), `scrollIntoView` к выделению.
...
### Feat: фильтр по статусу валидации на экране внутридокументного аудита

На экран аудита (web, основной интерфейс) добавлен третий фильтр — по статусу валидации находки (К валидации / Подтверждено / Игнорировать / Исправлено / Ложное срабатывание). Опции строятся из реально присутствующих в находках статусов (как фильтр по типу), фильтрация клиентская — рядом с фильтрами серьёзности и типа. Только `apps/web/src/app/(app)/audit/[docVersionId]/page.tsx`. Требует деплой web.
...
- `audit.getDocumentSections` тянул в память **весь `rawHtml`** каждого контент-блока документа, хотя в ответ идёт только `content` → заменил `include` на `select { content }` (форма ответа не изменилась);
- `audit.getAuditFindings` в непагинированном режиме (так его зовёт web) грузил **все** находки без потолка → добавлен защитный backstop 2000 (реальные документы с сотнями находок не урезаются; патология не растит heap безгранично) + warning-лог при срабатывании;
- `/api/audit-report/:versionId` буферизует весь Word-документ в памяти → ограничил выборку находок потолком 5000 + warning-лог.
...
### Fix: устойчивость QA-шага intra-audit к падению отдельных вызовов (fetch failed)

Аналогично llm_check: на шаге llm_qa ошибка одного вызова (`fetch failed`) пробрасывалась во внешний catch и обрывала остальные QA-батчи. Теперь каждый вызов `runQaBatch` (Strategy A — один вызов, Strategy B — цикл батчей) обёрнут в try/catch: вердикты успешных батчей применяются сразу (не теряются), упавший батч пропускается, следующие продолжают выполняться. Упавшие вызовы (`qa_single` / `qa_batch:i/N` + ошибка) пишутся в `ProcessingStep.result.failedCalls` и отображаются в «Аудит обработок» тем же блоком, что и для llm_check (#159). Только `apps/workers/src/handlers/intra-doc-audit.ts`. 237 workers-тестов проходят. Требует деплой workers.
...
### Fix: фильтры по серьёзности и категории на экране внутридокументного аудита (основной интерфейс)

Оба фильтра на экране `audit/[docVersionId]` (apps/web) не работали:
- **Серьёзность**: фильтр уходил на бэк (`getAuditFindings({ severity })`), который фильтрует по колонке `Finding.severity`. Но handler intra-audit при создании находки эту колонку не заполняет — серьёзность лежит только в `extraAttributes.severity`. Колонка = NULL → любой выбор серьёзности обнулял список. Плюс в селекте была опция «Critical», которой в данных нет (`mapSeverity` сворачивает `Critical → high`).
- **Категория**: клиентская фильтрация по `extraAttributes.auditCategory`, которое handler никогда не пишет; таксономия селекта (consistency/logic/terminology/...) в данных не существует → любая категория давала пусто.

Фикс: оба фильтра теперь **клиентские**, по `extractFindingMeta(f)` — тем же значениям, что показаны на карточках находок (severity из `extra.severity`, type из колонки `type`). `severity` больше не передаётся на бэк. Опция «Critical» убрана. Фильтр «категория» заменён на **«тип находки»** (Семантическая / Редакторская — поле `type`, заполнено у всех находок).

### Fix: intra-audit QA давал 0 вердиктов — лимит находок на QA-вызов

Корень (диагностирован по логам прода: `Batch 1/1, findings: 216 → Complete {0,0,0}`, без ошибок парсинга): `batchFindings` делил находки по input-бюджету (символам описаний), но **не ограничивал их количество**. 216 коротких находок влезали в input одним батчем, но столько же вердиктов + reasoning deepseek-v32 **не влезали в `maxOutputTokens` (25000)** → ответ обрезался → `unwrapJsonArray` не находил валидный `{"verdicts":[...]}` → возвращал `null` → цикл сопоставления пропускался → тихие 0/0/0 (новый `unwrapJsonArray` не бросает исключение, поэтому в логах не было `Failed to parse`). Это не регрессия wrapper-фикса (#148) и не промт — промт version 4 корректный.

Фикс (`apps/workers/src/handlers/intra-doc-audit.ts`):
- **`MAX_FINDINGS_PER_QA_BATCH = 20`** — жёсткий лимит числа находок в одном QA-вызове. Применён и в `batchFindings` (режем батч по бюджету ИЛИ по числу, что раньше), и в Strategy A (single-call только если находок ≤ лимита). 216 находок → ~11 батчей, каждый легко влезает в 25k output.
- **Устойчивость к обрезанному JSON**: если `unwrapJsonArray` вернул пусто — fallback `extractVerdictObjects` вытаскивает уцелевшие вердикты поштучно (`\{[^{}]*\}` + проверка полей `id`/`verdict`), чтобы не терять весь батч при частичной обрезке.
- **Диагностика** (убрано слепое пятно): лог при 0 распарсенных вердиктов (образец сырого ответа), лог при «вердикты есть, но ни один не сопоставился по id».

237 workers-тестов проходят. Требует деплой workers (ре-сид промтов НЕ нужен — промт уже корректный).
...
Фикс: убран `responseFormat: "json"` в 3 вызовах intra-audit (Variant 1, Variant 2, QA). Промты явно требуют «строго JSON-массив», парсеры толерантно извлекают `[...]` (стрипают `<think>`). Теперь intra-audit возвращает все находки независимо от выбранной модели.

### Fix: intra-audit — очистка находок прошлых прогонов (расхождение счётчиков)

Баг: счётчик «LLM находки: N» расходился с QA («Всего: M», M>N). Причина: handler не удалял находки прошлых прогонов, QA грузит все `pending` документа → накопление. Фикс: `deleteMany({ docVersionId, type in [editorial, semantic] })` в начале детерминированного шага. FindingReviewLog ссылается без FK-cascade, удаление безопасно.
...
Новая раскладка:
- **Строка 1:** `[← Назад]  Парсинг и разметка                  [↻]`
- **Строка 2:** `имя_документа — v1            [193]  [+]`

Имя документа теперь single-line с ellipsis + `title` атрибут (полный текст при hover). Действия и счётчик размещаются на нижней строке, всегда видны.
...
- Добавлен второй select в шапке рядом с фильтром этапов: «Все протоколы» + список уникальных samples из текущего queue.
- Уникальные samples собираются из `items` (id → name, отсортированы по name). Показываются только те, у которых **есть** открытые вопросы — пустой выпадающий список = нет вопросов.
- `filteredItems` применяет оба фильтра одновременно (стейдж AND протокол).
- Удобно когда вопросов 100+ и эксперт хочет разобрать всё по одному протоколу за раз.
...
- Добавлена 4-я кнопка **«Не могу определить зону» (U)** рядом с «Вопрос эксперту». Создаёт annotation с `isQuestion=true` и фиксированным текстом «Не могу выбрать зону, оставляю выбор за экспертом.» — без модального окна, мгновенно. Ускоряет workflow для случаев когда annotator не имеет идей и не хочет писать вопрос вручную.
- Хоткей `U` добавлен в hotkey-модалку и handler.
- Inline-реализация SourcePanel удалена → импорт из `@/components/SourcePanel`. Поведение идентично, но ширина шире.

`apps/rule-admin/src/app/(app)/expert-review/page.tsx`:
- Добавлена тогл-кнопка **«Исходник»** (`Columns2`) в шапке (как у annotator).
- При включении и выбранном вопросе — справа открывается панель `SourcePanel` шириной 40rem с разделами выбранного sample. Активный раздел (по `sectionKey === title.trim().toLowerCase()`) подсвечен и автоматически проскроллен наверх.
- tRPC-запросы `goldenDataset.getSample` + `document.getVersion` активны только при `showSource && selectedItem` — без неоправданных вызовов API.

Оба экрана теперь используют общий компонент. Для эксперта особенно важно — раньше при разборе question приходилось переходить в /golden-dataset, чтобы прочитать содержимое раздела; теперь контекст рядом.
