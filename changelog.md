...
- **Страница «Аудит обработок» в Rule Admin** — новая страница `/audit` отображает все обработки тенанта с фильтрами по типу и статусу. На уровне обработки показывается использованный Rule Bundle, на уровне каждого этапа (раскрытие строки) — LLM-конфигурация (провайдер, модель, температура) и snapshot применённых правил
