...
### Feat: word add-in — паритет интерфейса находок writer с reviewer

Перенесены в writer-страницу «Находки (внутридокументный аудит)» возможности, уже бывшие у reviewer:
- **Поиск по тексту находки** (по описанию, рекомендации, цитатам) — `Input` в шапке списка; устойчивость выбора при наборе (находка ищется в полном списке, деталь не сбрасывается).
- **Навигация по цитатам как у reviewer**: автопереход на первое место при открытии находки, клик по каждой цитате для перехода, несколько мест с цветными левыми полосами и подсветкой активной.
- **Переход именно в нужный раздел** (`sectionHeadingFor` → `navigateToText(sectionHint)`) — больше не прыгает в первое одноимённое вхождение по документу. Разделы подгружаются через `audit.getDocumentSections`.
- **Зоны находки** `anchor → target` для cross-находок в детализации.
- **flexShrink:0 на бейджах** карточки и детали — тот же фикс обрезки, что #201.
- Фильтры и bulk-кнопки скрываются в режиме детали (как у reviewer).

Требует деплой word-addin.

### Fix: word add-in — пустой зазор между фильтрами и списком находок (writer)

На странице «Находки (внутридокументный аудит)» writer-режима между фильтрами и списком висела пустая полоса: standalone Fluent `<Divider/>` имеет дефолтный `minHeight: 20px`. Divider убран, разделение списка/детали сделано тонким `borderTop` (как уже у detail-панели), верхний паддинг списка выровнен.

Требует деплой word-addin.

### Fix: подсветка цитат в HTML-разделе находки (web + rule-admin)

После перевода вывода разделов в форматированный HTML (#189/#195) подсветка текста цитаты перестала работать в обоих интерфейсах работы с находками: при наличии `contentHtml` `SectionPanel` рендерил HTML напрямую через `dangerouslySetInnerHTML`, а жёлтая подсветка осталась только в мёртвом plain-text-режиме. Добавлена функция `highlightHtml`, которая подсвечивает фрагменты цитат **прямо в HTML, не ломая разметку**: обрабатываются только текстовые узлы (между тегами), совпадения оборачиваются в `<mark>` (стиль через `[&_mark]:` в `HTML_CONTENT_CLASSES`). Теги/атрибуты не трогаются; цитата, разорванная тегом (ячейки таблицы, переносы), может не подсветиться — допустимый компромисс ради сохранности таблиц. Правка в `apps/web` и `apps/rule-admin` (`finding-display.tsx`).

Требует деплой web + rule-admin.

### Fix: расхождение счётчиков «Видимые/Скрытые» на экране finding-review

В шапке экрана `finding-review/[id]` счётчики «Видимые/Скрытые» считались только по `hiddenByReviewer`, а фильтры «Не ложные (видимые)» / «Ложное срабатывание» — по `hiddenByReviewer || status==="false_positive"`. Из-за этого находки со `status=false_positive`, не скрытые ревьюером вручную, попадали в «Видимые» в шапке, но в «Ложное срабатывание» в фильтре — визуально «терялось» N находок (Видимые 56 vs «Не ложные (видимые)» 51; Скрытые 127 vs «Ложное срабатывание» 132). Введён единый предикат `isFalsePositive(f)`, используемый и в фильтре, и в счётчиках. Это совпадает с тем, что реально скрыто от писателя (`audit.service`: писатель не видит `hiddenByReviewer` И `status=false_positive`), — ярлык «Скрытые» честен.

Требует деплой rule-admin.

### Fix: word add-in — бейдж статуса обрезался на узкой панели

В карточке находки (`ReviewPanel`) ряд бейджей (`SeverityBadge`, `StatusBadge`, направление, «Ложное») — flex с переносом, но без `flexShrink:0`. При узкой ширине панели flexbox сжимал бейдж уже его текста, а Fluent `Badge` обрезает содержимое по `overflow:hidden` — «К валидации» был виден частично. Добавлен `"& > *": { flexShrink: 0 }` в стиль `badges`: бейджи переносятся целиком, без обрезки.

Требует деплой word-addin.

### Fix: word add-in — переход по цитате садится на НАЧАЛО цитаты

Раньше выделение часто начиналось с середины/конца цитаты: `Word.body.search` не пересекает границы абзаца/ячейки, поэтому при многоабзацной цитате или «склейке» (пропущенный пробел) длинные кандидаты не находились и побеждал суффикс (последние 60 символов). Переработана генерация кандидатов в `navigateToText`:
- старт-выровненные префиксы перебираются longest-first (`200…16`) — выделение берёт максимальный кусок **от начала** цитаты, влезающий в один абзац;
- если самый старт «битый», пробуются фрагменты от первых словесных границ по возрастанию смещения (ближе к началу), а не хвост;
- суффикс остаётся лишь крайним случаем.

Требует деплой word-addin.

### Feat: отчёт внутридокументного аудита — раздел находки + страница на находку

Отчёт `/api/audit-report` переработан, чтобы по каждой находке было понятно, где она возникла:
- **Каждая находка — с новой страницы** (`pageBreakBefore`).
- **Явный раздел** находки с **исходной нумерацией Word** (`Section.headingNumber`, напр. «5.3.1 Этапы исследования»). Раздел определяется по цитате (контент раздела содержит цитату) с предпочтением зоны находки; иначе — по зоне.
- Богатый блок как в web: серьёзность, тип/категория/подтип/семейство, статус, зоны (anchor→target), рекомендация, цитаты из документа раздельно с подписями (Цитата/Референс/Якорная/Проверяемая зона).
- Endpoint подгружает разделы (title/standardSection/headingNumber + текст блоков для сопоставления; без `rawHtml` — защита от OOM).

Требует деплой api.

### Feat/Fix: список находок — поиск по тексту, удержание выбора, выравнивание панели

- **Поиск по тексту** добавлен как фильтр в список находок: Word-аддон (ревью), web-аудит (writer) и rule-admin finding-review. Ищет по описанию, рекомендации, цитатам (и заметке ревьюера в rule-admin), без регистра.
- **Word: смена критичности не сбрасывает выбор** — выбранная находка определяется по id из полного списка, а не из отфильтрованного; раньше при активном фильтре по severity смена критичности «перескакивала»/сбрасывала детализацию.
- **Выравнивание панели детализации** по правому краю окна: убран `max-w-3xl` на экранах внутридокументного аудита (web) и ревью (rule-admin) — детализация занимает всю ширину правой панели.

Требует деплой web + rule-admin + rebuild word-addin.
...
### Fix: Word add-in — переход к тексту для находок на стыке ячеек/абзацев

Для части находок «Перейти к цитате» не срабатывал: цитата пересекала границу ячейки/абзаца таблицы (склейка вида «…ОрганизацияООО «…»» — та самая «нет пробела»), и такой непрерывной строки в документе нет → `body.search` не находил. `navigateToText` теперь спускается по более коротким префиксам (255→24) и дополнительно пробует суффикс (последние 60 символов) — ловит часть цитаты до/после «битого» стыка; выбор вхождения по-прежнему привязан к разделу. Требует rebuild word-addin.
