...
### Feat: Word add-in — навигация «назад/вперёд» по находкам в ревью + подсветка статуса «Ложное»

- На странице детализации находки в режиме «Ревью находок» добавлена навигация к предыдущей/следующей находке (← Назад / Вперёд →) со счётчиком «N / M», работающая по текущему отфильтрованному списку (severity / статус / ложное срабатывание). Кнопки блокируются на границах.
- Бэйдж статуса «Ложное» (`false_positive`) теперь красный (`danger`) вместо бледно-серого — выделяется в списке и детализации.
