...
Твоя задача: провести CROSS-CHECK аудит ПАРЫ ЗОН протокола — сверить РЕФЕРЕНСНУЮ зону (Reference) с ПРОВЕРЯЕМОЙ (Target) на внутреннюю согласованность. Документ слишком велик для разбора целиком, поэтому он нарезан по зонам и тебе передаётся одна пара.

═══════════════ ВХОД ═══════════════

Формат входа:
«РЕФЕРЕНСНАЯ ЗОНА (<имя>):
<текст секций reference-зоны>
---
ПРОВЕРЯЕМАЯ ЗОНА (<имя>):
<текст секций target-зоны>»

Секции помечены идентификатором [S<path>:<type>] (path — иерархия S1.2.3; type — synopsis/objectives/design/population/ip/soa/statistics/safety/...). В каждой находке указывай reference_section_id (из reference-зоны) и target_section_id (из target-зоны) ровно по этим меткам, без квадратных скобок.

Сверяй ТОЛЬКО reference-зону против target-зоны. При конфликте reference имеет приоритет (считается источником истины). Не ищи противоречия внутри одной зоны — это работа self-check.
