/**
 * Детерминированный поиск и выделение таблицы SOA (Schedule of Activities).
 *
 * Алгоритм работает в 5 фаз:
 *   1. Обнаружение — сбор сигналов из заголовка и содержимого таблицы
 *   2. Скоринг — количественная оценка (0..~20)
 *   3. Решение — пороговая логика для определения SOA
 *   4. Извлечение — конвертация в строки, сбор примечаний
 *   5. Нормализация — выделение заголовочной строки, унификация маркеров
 *
 * Поддерживает объединённые ячейки заголовков (colspan/rowspan):
 *   «Период терапии» (colspan=4) → «Визит 3 | Визит 4 | Визит 5 | Визит 6»
 *   → «7±1 | 14±1 | 21±1 | 28±1» — составляются многоуровневые имена визитов.
 *
 * Работает с HTML-таблицами (выход mammoth), адаптация алгоритма из DOCX.
 */
...
interface TableCandidate {
  ...
  rawHtml: string;
  title: string;
  /** Raw parsed rows preserving colspan/rowspan */
  htmlRows: HtmlCell[][];
  /** Fully expanded grid (each cell occupies exactly 1 position) */
  rows: string[][];
  sectionId: string;
  order: number;
}
...
interface SoaDetectionResult {
  ...
  score: number;
  visits: string[];
  /** Multi-level header rows for UI display (each row is an array of {text, span}) */
...
/**
 * Detect how many top rows form the header.
 * Header rows typically contain timepoint labels, not X-markers.
 * We look at the first N rows and find where data rows start.
 */
function detectHeaderRowCount(rows: string[][]): number {
  ...
  for (let i = 1; i < limit; i++) {
    ...
    // If significant portion of cells are X-markers or footnotes, data starts here
    if (dataCells.length > 0 && (xCount + footnoteCount) / dataCells.length > 0.3) {
      return i;
    }

    // If first column looks like a procedure name, data starts here
  ...
  // Fallback: use timepoint scoring
...
/**
 * Build multi-level visit names from header rows.
 * Handles merged cells: e.g., "Период терапии" spanning 4 cols
 * with "Визит 3 | Визит 4 | Визит 5 | Визит 6" below it
 * and "7±1 | 14±1 | 21±1 | 28±1" in the third row.
 *
 * Leaf-level visit names become composite: "Период терапии / Визит 3 / 7±1"
 */
function buildMultiLevelVisits(
  ...
  // Build a grid of header cells preserving span info
  // We use the htmlRows (with colspan) to reconstruct the hierarchy
  ...
  // Build leaf-level (last header row) column-to-parent mapping
  // For each leaf column (col index 1..numCols-1), compose a name
  // by collecting text from each header level
  const expandedHeader: string[][] = [];
  for (let r = 0; r < headerRowCount && r < expandedRows.length; r++) {
    expandedHeader.push(expandedRows[r]);
  }

  // For leaf columns (skipping col 0 = procedure label), compose visit names
  const visits: string[] = [];
  ...
  // Build headerLevels for UI display (skip col 0)
  ...
  for (let r = 0; r < headerGrid.length; r++) {
    ...
    for (const cell of headerGrid[r]) {
      if (colsConsumed === 0) {
        // First cell in row is the procedure column header — skip it for visit headers
...
function normalizeMarker(raw: string): string {
  ...
  if (DASH_MARKER_RE.test(raw)) return "\u2013"; // en-dash
  return raw;
}
...
/**
 * Expand HTML rows with colspan/rowspan into a flat grid
 * where each cell occupies exactly one position.
 * Merged cells are filled with the parent cell's text.
 */
function expandGridFromHtmlRows(htmlRows: HtmlCell[][]): string[][] {
  ...
  // Initialize grid with null markers
  ...
  for (let r = 0; r < numRows; r++) {
    let gridCol = 0;
    for (const cell of htmlRows[r]) {
      // Find next empty position in this row
      while (gridCol < maxCols && grid[r][gridCol] !== null) {
        gridCol++;
      }
      if (gridCol >= maxCols) break;

      // Fill the grid positions for this cell's colspan x rowspan
  ...
  // Replace any remaining nulls with empty string
