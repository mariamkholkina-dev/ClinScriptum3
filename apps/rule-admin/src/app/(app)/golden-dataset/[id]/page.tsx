              ...
              {!isEditing ? (
                <div className="flex items-center gap-2">
                  {canGenerate && (
                    <button
                      onClick={() => {
                        // Sprint 7c: явное предупреждение. Раньше эксперт мог
                        // случайно нажать кнопку и потерять все точечные правки
                        // эталона (incident 2026-05-05 — артефактный baseline 0.902).
                        const confirmed = window.confirm(
                          "⚠ ВНИМАНИЕ\n\n" +
                            "Эта операция перезапишет ВЕСЬ эталон текущим состоянием разметки.\n\n" +
                            "ВСЕ ваши предыдущие точечные правки эталона будут потеряны.\n\n" +
                            "Используйте только при ПЕРВОНАЧАЛЬНОМ создании эталона. " +
                            "Для точечных правок используйте dropdown в строке секции — " +
                            "правка теперь синхронизируется с эталоном автоматически.\n\n" +
                            "Продолжить?",
                        );
                        if (confirmed) generateExpectedJson();
                      }}
                      className="flex items-center gap-1 text-xs text-amber-600 hover:text-amber-700"
                      title="Перезаписать весь эталон текущим состоянием. Точечные правки теряются — для них используйте dropdown в дереве."
                    >
