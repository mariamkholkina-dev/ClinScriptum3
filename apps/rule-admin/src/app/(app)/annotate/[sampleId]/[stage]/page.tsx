...
export default function AnnotatePage() {
  ...
  const CANT_DETERMINE_TEXT = "Не могу выбрать зону, оставляю выбор за экспертом.";
  const handleCantDetermine = () => {
    submit({ question: true, questionText: CANT_DETERMINE_TEXT });
  };
  ...
  return (
    <div className="flex h-screen flex-col bg-gray-50">
      ...
      <header className="flex items-center justify-between border-b border-gray-200 bg-white px-6 py-3">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            title="Показать/скрыть панель «Исходник» (содержимое разделов)"
          >
      ...
      <div className="flex flex-1 overflow-hidden">
              ...
              {!questionMode ? (
                <div className="mt-6 rounded-lg border border-gray-200 bg-white p-4">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    ...
                    <button
                      onClick={handleCantDetermine}
                      disabled={submitMut.isPending}
                      className="flex items-center gap-1 rounded border border-amber-300 bg-white px-3 py-1.5 text-sm font-medium text-amber-700 hover:bg-amber-50 disabled:opacity-40"
                      title="Создать вопрос эксперту с пояснением «Не могу выбрать зону». Эксперт укажет финальную зону самостоятельно."
                    >
                      <CircleSlash size={14} /> Не могу определить зону (U)
                    </button>
                  </div>
                </div>
              ) : (
        ...
        {showSource && (
          <SourcePanel sections={sections} focusedSectionId={activeSection?.id ?? null} />
        )}
      </div>
        ...
        >
          <div className="w-96 rounded-lg bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <ul className="space-y-1 text-sm">
              ...
              <li><kbd className="rounded border bg-gray-100 px-1">U</kbd> — «Не могу определить зону» (auto-вопрос эксперту)</li>
